...
import { iUserData } from '../../src/domain/entities/interfaces/iUserData'
...
test('should throw an error if the password is less than 8 chars long', () => {
  ...
  expect(() => new User(userData)).toThrow(new Error("Invalid password: password less than 8 chars long"));
})

test('should throw an error if receives an invalid email', () => {
  userData.email = 'leandro.com'
  expect(() => new User(userData)).toThrow(new Error("Invalid email format"));
})

test('should throw an error if receives a first name of length zero', () => {
  userData.firstName = ''
  expect(() => new User(userData)).toThrow(new Error("Invalid string format"));
})
